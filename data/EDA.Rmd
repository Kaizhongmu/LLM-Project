...
-   **Column-wise Missingness:**\
    ...
    The *Specialty* column, which reflects sub-identity of corresponding commenter's role , is missing in 77% of entries.\
    The *Other* column, which captures other extractable information from comments, has a 77% missing rate.\
...
df[,8:13]=sapply(df[,8:13],as.factor)
df[,3]=sapply(df[,3],as.factor)

# add word counts into the dataset 
...
# add state 
df=merge(df,original_data[,c("Document ID","State/Province")],by.x = "Document.ID", by.y = "Document ID",all.x = TRUE)
...
-   **Description:**\
    The **tone** of a comment is categorized as *very_negative*, *negative*, *neutral*, or *positive*.\
    The **has_substantive_tone** of a comment indicates whether there is a substantive explanation rather than just expressing emotions.\
    The **tone_justify** provides the reasoning behind this classification based on linguistic metrics. These include `negative_word` (presence of words with negative connotation like *not*, *destroy*), `strong_word` (forceful terms like *must*, *stop*), `imperative` (use of imperative sentences), `capitalization` (use of capitalization for emphasis), `punctuation` (presence of punctuation mark such as *!* or *?*), `implicature` (present of indirect expression or implied negativity ), `fact-based` (whether is fact-based), and `supportive` (whether the comment expresses encouragement or approval). `high_frequency` (it measures how often these features occur within the comment, either by calculating the proportion of sentences containing such features or noting repeated use of the same indicator).\

  

-   **Tone Distribution:**\
    "Percentage of Each Category in Tone" pie chart shows Negative and very negative tones have roughly equal proportions (44% and 47%, respectively). In contrast, neutral and positive tones are much less common (8% neutral, 1% positive). This indicates that the majority of people in the sample express a negative attitude toward PE.
...
-   **Explore has_substantive_tone :**\
    When summarizing the comments, we noticed that some of the comments did not contain much substantive information. Therefore, we are now examining the characteristics of those comments that lack substantive content, including tone, comment role, and concern level. we find most of them are very_negative tone. Most of them are shows no_concern level, All of them don't show the commenter role. In conclusion, Non-substantive_tone comment would like to contain less useful information. 
```{r,echo=FALSE, message=FALSE,warning=FALSE,results='asis'}
...
pie_tone <- df %>%
  ...
  mutate(pct = round(100 * n / sum(n), 1),
         label = paste0(Tone, ": ", pct, "%"))
blue_palette <- colorRampPalette(c("#deebf7", "steelblue"))
colors <- blue_palette(nrow(pie_tone))
plot_ly(pie_tone,
        x = ~Tone,
        y = ~pct,
        type = 'bar',
  ...
  layout(title = "Percentage of Each Category in Tone",
         xaxis = list(title = "Tone Category"),
...
######################################################################
df_sub <- df_percentage_long %>% 
  filter(Proportion > 0)
blue_palette <- colorRampPalette(c("#deebf7", "steelblue"))
unique_justifications <- unique(df_sub$Tone_justify)
colors <- setNames(blue_palette(length(unique_justifications)), unique_justifications)
plot_ly(data = df_sub, 
        x = ~Tone, 
        y = ~Proportion, 
        color = ~Tone_justify, 
  ...
  layout(barmode = "stack",
         title = "Distribution of Tone_justify in Each Tone Category",
         xaxis = list(title = "Tone"),
         yaxis = list(title = "Proportion", ticksuffix = "%"),
         legend = list(title = list(text = "Tone Justification"), orientation = "v"))



######################################################################
#    Explore the not_has_substantive comments' characteristics.      # 
######################################################################

df_non_substantive <- df %>% filter(has_substantive_tone == 0)
tone_summary <- df_non_substantive %>%
  count(Tone) %>%
  mutate(Proportion = round(100 * n / sum(n), 1)) # Tone distribution in non_has_substantive_tone
role_summary <- df_non_substantive %>%
  count(Commenter_Role) %>%
  mutate(Proportion = round(100 * n / sum(n), 1)) # Commenter_Role distribution in non_has_substantive_tone
concern_summary <- df_non_substantive %>%
  count(Concern_Level) %>%
  mutate(Proportion = round(100 * n / sum(n), 1)) # Concern_Level distribution in non_has_substantive_tone
tone_summary <- tone_summary %>%
  mutate(CategoryGroup = "Tone",
         Category = Tone) %>%
  select(CategoryGroup, Category, Proportion)
role_summary <- role_summary %>%
  mutate(CategoryGroup = "Commenter Role",
         Category = Commenter_Role) %>%
  select(CategoryGroup, Category, Proportion)
concern_summary <- concern_summary %>%
  mutate(CategoryGroup = "Concern Level",
         Category = Concern_Level) %>%
  select(CategoryGroup, Category, Proportion)
summary_all <- bind_rows(tone_summary, role_summary, concern_summary)

#plot
summary_all <- summary_all %>%
  mutate(Category = ifelse(is.na(Category), "Missing", as.character(Category)))
blue_palette <- colorRampPalette(c("#deebf7", "steelblue"))
unique_categories <- unique(summary_all$Category)
colors <- setNames(blue_palette(length(unique_categories)), unique_categories)
...
plot_ly(summary_all,
        ...
        colors = colors,
        type = "bar",
        text = ~paste0(Category, ": ", Proportion, "%"),
        textposition = "auto") %>%
  layout(barmode = "stack",
         title = "Distribution of Tone / Role / Concern Level in Non-substantive-tone Comments",
         xaxis = list(title = ""),
         yaxis = list(title = "Proportion (%)"),
         legend = list(title = list(text = "Sub-category")))

```

## Tone Quote 
below are the examples of Tone Quote 
- **FTC-2024-0022-0951**: Very Negative
  > "It is WRONG and destructive to quality of care.","PLEASE DO NOT LET OUT OF CONTROL GREED DESTROY MEDICAL CARE IN THIS COUNTRY!!","Medicine should focus on CARE, not SHAREHOLDER OR OWNER PROFIT MAKING!!!"

- **FTC-2024-0022-1656**: Negative
  > "Private Equity firms, large insurance companies have made working in healthcare extremely unfulfilling with constant pressures to meet unattainable Patient volumes and constant cutting of staff and services with expectation. ","making it difficult to care"

- **FTC-2024-0022-1432**: Neutral
  > "hat really got us to where we are today is government regulation, Medicare reimbursement to physicians, and health insurance company mergers and concentration and their corresponding clout to increase requirements for payment and decreasing insurer reimbursement to physicians"
  
- **FTC-2024-0022-1703**: Positive
  > "private equity can increase access to medical care by increasing efficiency; to deem all private equity as “bad for medicine” would be too broad"
...
role_details <- df %>%
  ...
  summarise(
    Count = n(),
    Specialties = paste(unique(na.omit(Specialty)), collapse = ", ")
  ) %>%
  mutate(Specialties = ifelse(Specialties == "", "-", Specialties)) %>%
  ungroup() %>%
  gt() %>%
  tab_header(title = "Commenter Role with Specialties and Total Count")
...
blue_palette <- colorRampPalette(c("#deebf7", "steelblue"))
colors <- blue_palette(nrow(pie_role))
plot_ly(
  pie_role,
  x = ~Role_category,
  y = ~n,
  ...
  layout(
    title = "Distribution of General Role",
...
blue_palette <- colorRampPalette(c("#deebf7", "steelblue"))
colors <- blue_palette(max(nrow(pie_role_most), 2))  # ensure at least 2 colors
colors <- colors[1:nrow(pie_role_most)]              # trim if more than needed
plot_ly(
  pie_role_most,
  x = ~Commenter_Role,
  y = ~n,
  ...
  layout(
    title = paste0("Distribution of Commenter Roles in General Role: ", most),
    xaxis = list(title = "Commenter Role", tickangle = -45),
    yaxis = list(title = "Count"),
    margin = list(b = 100),  
    showlegend = FALSE
  )
...
```{r,echo=FALSE, message=FALSE,warning=FALSE,results='asis'}
####################################################################
## distribution of word count by concern level 
####################################################################
# table 
...
- **Description**\
we are interested in the relationship between state and cocern level. So, firstly I show the distribution of State. Then I construct a contingency table with heat map to counts the numbers entries in each categories (state vs concern level). Based on that construct the chi-square test to see if the correlation between state and concern level is significant. lastly, we want to see, for each concern level, the distribution of states. We also want to see, for each concern category, the distribution of states. 

- **Distribution of State**\
The four states that made the most comments are NY, IL, NC and FL.

- **significance of correlation**\
P-value is $0.1912$. So the sample does not show strong correlation between State and concern level 

- **Distribution of State in each concern level**\
  - For Highest concern level, UT,OH,MI,and CT contributes most, each with 25%
  - For High concern level,FL contributed most 
  - For Medium concern level, NY and NC contributed most 
  - For Low concern level, TX contributed most 

- **Distribution of State in each concern categories**\
  - For `Patient_Cost`, FL and NC contribute most, each with 13.3%
  - For `Patient_quality`,FL,IL,NC,and NY contribute most, each with 8.3%
  - For `Provider_pay`, CT, IL,MD,NY,MI,OH,and UT contributed most, each with 14.3%
  - For `Porvider_quality`, NY,IL,FL contribute most (12.8%,7.7%,7.7%)
```{r,echo=FALSE, message=FALSE,warning=FALSE,results='asis'}
######################################################################
# Step 1: Distribution of State/Province
######################################################################
state_dist <- df %>%
  count(`State/Province`) %>%
  mutate(pct = round(100 * n / sum(n), 1),
         label = paste0(`State/Province`, ": ", pct, "%"))
blue_palette <- colorRampPalette(c("#deebf7", "steelblue"))
colors <- blue_palette(nrow(state_dist))
plot_ly(state_dist,
        x = ~`State/Province`,
        y = ~pct,
        type = 'bar',
        ...
        textposition = 'auto',
        marker = list(color = colors)) %>%
  layout(title = "Distribution of State/Province",
         xaxis = list(title = "State/Province", tickangle = -45),
         yaxis = list(title = "Percentage (%)"),
         margin = list(b = 100))


######################################################################
# Step 2: Heatmap of raw counts
######################################################################
df_clean <- df %>%
  filter(!is.na(`State/Province`), !is.na(Concern_Level)) %>%
  mutate(
    `State/Province` = str_trim(`State/Province`),
    Concern_Level = str_trim(Concern_Level)
  )
df_heat <- df_clean %>%
  count(`State/Province`, Concern_Level)

ggplot(df_heat, aes(x = Concern_Level, y = `State/Province`, fill = n)) +
  geom_tile(color = "white") +
  geom_text(aes(label = n), color = "black") +
  scale_fill_gradient(low = "white", high = "steelblue") +
  theme_minimal() +
  labs(title = "Heatmap of Concern Level by State/Province",
       x = "Concern Level",
       y = "State/Province",
       fill = "Count") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
df_table <- df_clean %>%
  count(`State/Province`, Concern_Level) %>%
  pivot_wider(names_from = Concern_Level,
              values_from = n,
              values_fill = 0)
contingency_matrix <- df_table %>% select(-`State/Province`) %>% as.matrix()
rownames(contingency_matrix) <- df_table$`State/Province`
...
######################################################################
# Step 4: distribution of state in each concern level 
######################################################################
df_percent <- df %>%
  filter(!is.na(`State/Province`) & !is.na(Concern_Level)) %>%
  group_by(Concern_Level, `State/Province`) %>%
...
states <- unique(df_percent$`State/Province`)
blue_palette <- colorRampPalette(c("#deebf7", "steelblue"))
colors <- setNames(blue_palette(length(states)), states)
plot_ly(
  data = df_percent,
  x = ~Concern_Level,
  y = ~Proportion,
  ...
  colors = colors,
  text = ~paste0(`State/Province`, ": ", round(100 * Proportion, 1), "%"),
  hoverinfo = 'text'
) %>%
  layout(
    barmode = 'stack',
    title = "Distribution of States within Each Concern Level",
    xaxis = list(title = "Concern Level"),
    yaxis = list(title = "Proportion", ticksuffix = "%"),
...
######################################################################
# Step 4: distribution of state in each concern category  
######################################################################
df_long <- df %>%
  select(`State/Province`, Patient_cost, Patient_quality, Provider_pay, Provider_quality) %>%
  pivot_longer(
    cols = c(Patient_cost, Patient_quality, Provider_pay, Provider_quality),
    names_to = "Concern_Type",
    values_to = "Concern_Flag"
  ) %>%
  filter(Concern_Flag == 1 & !is.na(`State/Province`))
df_count <- df_long %>%
  group_by(Concern_Type, `State/Province`) %>%
  summarise(Count = n(), .groups = "drop")
df_percent <- df_count %>%
  group_by(Concern_Type) %>%
...
plot_ly(
  ...
  layout(
    barmode = 'stack',
    title = "State Composition within Each Concern Type",
    xaxis = list(title = "Concern Type"),
    yaxis = list(title = "Proportion", ticksuffix = "%"),
    legend = list(title = list(text = "State"))
  )
```
